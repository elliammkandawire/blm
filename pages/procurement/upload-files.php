<?php
	 ...
	 include'send-document-mail.php';
...
<div class="wrapper">
  ...
  <span class="sr-only">Still sending email please wait...</span>
  </div>
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open">
            ...
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="upload-files.php" class="nav-link active">
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Upload and Send Document</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="home">Home</a></li>
              <li class="breadcrumb-item active">Upload and Send Document</li>
            </ol>
          </div>
        </div><!-- /.row -->
        <?php if($sess)
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Upload and Send Document</h3>
                    ...
                    if(isset($_FILES['file']) && isset($_POST['submit']))
	                {
                      ...
                      /*$maxsize = (5 * 1024) * 1024;
		                ...
		                if(!empty($_FILES['file']['name'])){
                            $sql = "SELECT file_id FROM upload WHERE file_name = '$filename'";
                            $results = mysqli_query($connection, $sql) or die(mysqli_error($connection));
                            if(mysqli_num_rows($results)>0){
                                $msg[] = "The File name already exist";
                            }
                        }
		                if(!in_array($extension,["doc","DOC","docx","DOCX","rtf", "RTF","pdf","PDF"]))
		                {
			                $msg[] = "The selected file is not of valid format";
						    $msg[] = "Please upload word document and or pdf files only";
		                }
						if(file_exists('uploads/' . $filename))
						{
							$msg[] = $filename . " already exists, Please upload a file with a different name";
						}
		                else
		                {
			                if(!$msg)
						    { 
			                    ...
			                    {
									$mailsend = sendmail($email, $subject, $message, $location);
                                    if($mailsend==1)
			                        {
				                        $insert = "INSERT INTO upload(file_name, size, date_uploaded, team_code, user_id)
				                        VALUES('$filename', '$size', Now(), '$teamCode', '$userid')";
					                    ...
					                    {  
					                        $success[] = "The email with an attachment has been sent successfully!";	
					                    }					
			                        }
									else
									{
										$msg[] = 'Failed to send an email due to network problem or the entered email is not reacheable';
									}
								}
						    }
		                }	
            ...
            <div class="card-body">
               <!-- form start -->
                <form id="orderForm" class="orderForm" action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
                   
				  <div class="form-row">
				     <div class="col-md-4 form-group">
				       <label for="supplierName">Suplier Name:</label>
                       <input type="text" name="supplierName" class="form-control" id="supplierName" placeholder="Enter supplier's name" data-rule-required="true" data-msg-required="Please enter supplier name" />
                     </div>
                     <div class="col-md-4 form-group">
				       <label for="firstname">Email:</label>
                       <input type="email" name="email" class="form-control" id="email" placeholder="Enter Supplier's email" data-rule-required="true" data-msg-required="Please enter supplier email" />
                     </div>
					 <div class="col-md-4 form-group">
				       <label for="supplierName">Subject:</label>
                       <input type="text" name="subject" class="form-control" id="subject" placeholder="Enter Subject" data-rule-required="true" data-msg-required="Please type the subject of the email" />
                     </div>
			      </div>
				  <div class="form-row">
                     <div class="col-md-12 form-group">
                      <label for="description">Message:</label>
                      <textarea class="form-control" name="message" id="message" rows="4" placeholder="Type the message"  data-rule-required="true" data-msg-required="Please type the message of the email"></textarea>
                    </div>
			      </div>
				  <div class="form-row">
				   <div class="col-md-12 form-group">
                    ...
                    <div class="input-group">
                      <div class="custom-file">
                        <input type="file" name="file" data-max-size="5242880" class="custom-file-input" id="file">
                ...
                <div class="card-footer">
                    <button type="submit" name="submit" id="submit" class="btn btn-primary"><i class="fa fa-upload"></i>&nbsp;Send File</button>
				</div>
				<!-- / .card footer -->
              </form>
            </div>
...
<script src="../../dist/js/checkUploads.js"></script>
<script src="../../dist/js/delete.js"></script>
<script>
$(function () {
