<?php
     ...
     $sess = array();
    if (!isset ($_SESSION['procurement-officer']))
        {
		    ...
		    if ($now > $_SESSION['expire'])
        {
	      ...
	      $user_check = $_SESSION['procurement-officer']; // Stored Session for current logged user
        ...
        if(isset($_GET['requisition_id']))
        {
			...
			//$query=(mysqli_query($connection,"UPDATE requisition SET status='Replied', date_replied=NOW() WHERE requisition_id=$requisitionId"));
...
<head>
  ...
  <title>Banja | Procurement Officer | View Requisitions</title>
...
<div class="wrapper">
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item dropdown">
        <a title="Messages" class="nav-link" data-toggle="dropdown" href="#">
          ...
          <?php
               $query = mysqli_query($connection, "SELECT * FROM requisition WHERE reply_status='Replied' AND view_status=3 AND comment3 !='Waiting for Approval' ORDER BY date_requested DESC LIMIT 4"); 
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    <!-- Brand Logo -->
    <a href="procurement-officer-dash.php" class="brand-link">
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="procurement-officer-dash.php">Home</a></li>
            ...
            <div class="card-body">
			    ...
			    <?php
                             ...
                             if(!$msg)
                             {
                                $update=(mysqli_query($connection,
								                      "UPDATE requisition 
			                                           ...
			                                           ON requisition.requisition_id=requisition_details.requisition_id 
			                                           SET requisition.status='Approved', requisition.view_status=4, 
			                                           requisition.date_replied=NOW(), requisition.comment4='$comment', 
			                                           requisition_details.reply_status=4 
	                                                   WHERE requisition.requisition_id='$requisitionId'"));
                                if (!$update)
                                {
                                  $msg[] = "Failed to accept requisition";
                                }
                                else
                                {
                                  $success[] = "The requisition has been approved successfully!";
                                }
                             }                            
                        }
                        if(isset($_POST['deny']))
                        {
                            ...
                            $update1=(mysqli_query($connection,
							                         "UPDATE requisition 
	                                                  JOIN requisition_details
			                                          ON requisition.requisition_id=requisition_details.requisition_id 
			                                          SET requisition.status='Denied', requisition.view_status=4, 
			                                          requisition.date_replied=NOW(), requisition.comment4='$comment', 
			                                          requisition_details.reply_status=4 
	                                                  WHERE requisition.requisition_id='$requisitionId'"));
                              if (!$update1)
                              {
                                $msg[] = "Failed to deny requisition";
                              }
                              else
                              {
                                $success[] = "The requisition has been Denied successfully!";
                              }                       
                        }
                        if($success)
                        { 
                    ...
                    ?>
               <div>
                  <form id="orderForm" action="<?php $_SERVER['PHP_SELF']; ?>" method="POST">
				    <div class="form-row">
					  <div class="col-md-6 form-group">
					   <input type="hidden" name="requisitionId" id="requisitionId" value="<?php echo $requisitionId;?>" id="requisitionId">
                      </div>
					</div>
					<div class="form-row">
					 <div class="col-md-12 form-group">
                       <label for="description">Comment</label>
                       <textarea class="form-control" name="comment" id="comment" rows="2" placeholder="Provide a comment"  data-rule-required="true" data-msg-required="Please provide a comment"></textarea>
                     </div>
                    </div>
				    <div class="form-row">
				     <div class="col-md-6 form-group">
                        <button type="submit" name="approve" id="approve" class="btn btn-primary"><i class="fa fa-check-circle"></i>&nbsp;Approve</button> 
						<button type="submit" name="deny" id="deny" class="btn btn-danger"><i class="fa fa-times-circle"></i>&nbsp;Deny</button> 
                     </div>
			       </div>
				  </form>
               </div>
              ...
              <!-- /.end table -->
                ...
                <div class="card-footer">
                  <nav aria-label="...">
                    ...
                    <?php if($page > 1){ ?>
                    <li class="page-item">
                        <a class="page-link" href="view-requisition.php?page=<?= $previous; ?>">Previous</a>
                      </li>
                      <?php 
                       } else { ?>
                      ...
                      <li class="page-item <?= $page == $i ? 'active':'' ?>" aria-current="page">
                            <a class="page-link" href="view-requisition.php?page=<?= $i; ?>"><?= $i; ?></a>
                      </li>
                      <?php endfor; ?>
                       
                      <?php if(($i > $page) && ($page < $pages)){ ?>
                      <li class="page-item">
                        <a class="page-link" href="view-requisition.php?page=<?= $next; ?>">Next</a>
                      </li>
                      <?php 
                       } else { ?>
                      <li class="page-item disabled">
