<?php
    ...
    if (!isset ($_SESSION['procurement']))
        ...
        $results = $connection->query("SELECT *  
									  ...
									  Where orderdetails.team_code=$teamCode
                                      ORDER BY orderDetails.order_date DESC LIMIT $start, $limit");
        $orders = $results->fetch_all(MYSQLI_ASSOC);
  
        $result1 = $connection->query("SELECT COUNT(order_id) AS id FROM orders");
               ...
               <table id="orderDetails" class="table table-striped table-advance table-bordered">
                ...
                ?>
                  <tr class="text-nowrap">
                    ...
                    <td><a href="team-detail.php?team_code=<?= $order['team_code']; ?>"><?= $order['team_code']; ?></a></td>
                    <td>
                       <div class="btn-group">
                          ...
                          <a class="btn btn-danger btn-sm deleteStock" href="#" id="<?=$order['order_id']; ?>" data-id="<?php echo $order['order_id']; ?>"><i class="fa fa-times">&nbsp;Delete</i></a>
                       </div>
                    </td>
                  </tr>
                  <?php 
                     ...
                     <?php 
                      ...
                      echo '<tr><td colspan="9" class="text text-danger font-weight-bold">There are currently no order details to display</td></tr>';
                    }?>
                </tbody>
              </table>
              <!-- /.end table -->
